...
### MapUnit Table
This table selects a given area of interst. For Entire SSURGO use "areasymbol  <> 'US'" OR STATSGO use "areasymbol = 'US'Areasymbol. This is an example from mukey = 2809839. Areasymbol = 'WI025' is pounded out with '--' .
...
### Component and Layer Table
###### acpf Table
This table collects map unit and component information needed. This includes all map units selected above and all components (typically soil series) within each map unit that do not have a limiting layer within 200cm. This example is limited to one mapunit, mukey = 2809839.  The information needed for further SOC stock calculations, individual layer information, is selected from each component.

Component variables used in SOC stock calculation (r denotes that the value is representative of the central tendency):
   * comppct_r = the proportion of each map unit made up of that soil component
...
Layer variables used in SOC stock calculation (r denotes that the value is representative of the central tendency):
   ...
   * hzdepb = bottom depth of layer
   * om_r = layer organic matter (%) content. US soil survey databases report OM (NSSH Part 618.43 )this is typically based on measurement of total combustion carbon and calcium carbonate concentration by calcimeter (KSSL, 2017). 
   * dbthirdbar_r = bulk density represting field conditions. US soil survey bases this on a clod measurement of volume at 1/3 bar and weight at oven dry water content (KSSL, 2014) 
   * fragvol_r = Volume of coarse fragments (>2mm in size), typically based on visual estimates
...
## Surface properties
These table evaluate surface textures.  Data attribution may be uneven for non-mineral layers.  Current implementaton begivs
